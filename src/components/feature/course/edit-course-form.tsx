...
export function EditCourseForm({
  ...
  const [coverImage] = useState<string>(course?.coverImage || "");
  ...
  useEffect(() => {
    ...
    if (!courseColor) {
      setCourseColor("#60A5FA"); // Default light blue color
    }
  }, [isNew, courseColor]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (
      courseStartDate &&
      courseEndDate &&
      new Date(courseStartDate) >= new Date(courseEndDate)
    ) {
    ...
    try {
      ...
      const requestData = {
        ...
        data: {
          ...
          courseColorStyle: courseColor
            ? createCourseColorStyle(courseColor)
            : null,
        },
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          ...
          <div>
            ...
            <p className="text-sm text-secondary-600 mt-1">
              Choose a color theme for your course. Light colors work best for
              accessibility.
            </p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="text-sm text-secondary-600 mb-2">
              Define the course units or modules. Drag rows to reorder. Click
              any cell to edit.
            </p>
            <SecondaryButton
              type="button"
          ...
          <div>
            ...
            <p className="text-secondary-600 text-sm text-gray-600 mb-2">
              Set up your course timeline with assignments, exams, and
              deadlines. Drag rows to reorder. Click any cell to edit.
            </p>
            <SecondaryButton
              type="button"
              className="mb-2 px-3 py-1 rounded transition-colors"
            ...
            <div className="border border-secondary-700 outline-secondary-700 rounded-lg overflow-hidden shadow-sm bg-white w-full">
              <Table
                data={timeline.map(
                  (item: CourseTimelineItem, index: number) => ({
                    ...item,
                    id: index,
        ...
        {!isNew && course && (
          <Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
            <DialogContent>
              ...
              />
              {deleteError && (
                <div className="text-danger-600 text-sm mt-2">
                  {deleteError}
                </div>
              )}
              <DialogFooter>
