      ...
      <div className="bg-primary-50 rounded-xl shadow border border-primary-100 p-4">
        ...
        <div className="space-y-2">
          {courseMaterials.map((material) => (
            <div key={material.id} className="flex items-center gap-3 p-2 hover:bg-gray-50 rounded-lg transition-colors group">
          ...
          {pollsAndAnnouncements.map((item) => (
            <div key={item.id} className="p-3 border border-gray-200 rounded-lg">
              <div className="flex items-start justify-between mb-2">
                ...
                <div className="flex items-center gap-2">
                  <span className={`px-2 py-1 text-xs rounded-full ${
                    item.type === 'poll' ? 'bg-blue-100 text-blue-700' : 'bg-orange-100 text-orange-700'
                  }`}>
                    {item.type}
                  </span>
                  {item.active && (
              ...
              <div className="flex items-center gap-2 text-xs text-gray-500">
                ...
                <span>{item.responses ? `${item.responses} responses` : 'No responses'}</span>
              </div>
            </div>
          ))}
        </div>
