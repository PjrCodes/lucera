...
export default function CourseGradesCard({ grades }: { grades: Grade[] }) {
  return (
    <div className="bg-primary-50 rounded-xl shadow border border-primary-100 p-6">
      <h2 className="text-lg font-semibold text-primary-900 mb-4 flex items-center gap-2"><FiBarChart2 />Grades</h2>
      <div className="space-y-3">
        {grades && grades.length > 0 ? grades.map((grade) => (
          <div key={grade.id} className="border-l-4 border-success-300 pl-3 py-2">
            <h4 className="text-sm font-medium text-primary-900 mb-1">{grade.title}</h4>
