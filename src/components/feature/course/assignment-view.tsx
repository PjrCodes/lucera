...
export default function AssignmentView({ assignment, course, onBack }: AssignmentViewProps) {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow border border-primary-100 p-6">
        <div className="flex items-start justify-between mb-4">
          <div>
            <h1 className="text-2xl font-bold text-primary-900 mb-2">{assignment.title}</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="bg-primary-50 p-4 rounded-lg">
            <h3 className="text-sm font-medium text-primary-500 mb-1">Start Date</h3>
            <p className="text-sm text-primary-900">{formatDate(assignment.startDate)}</p>
          </div>
          <div className="bg-primary-50 p-4 rounded-lg">
            <h3 className="text-sm font-medium text-primary-500 mb-1">Due Date</h3>
            <p className={`text-sm font-medium ${isOverdue ? 'text-red-600' : 'text-primary-900'}`}>
              {formatDate(assignment.dueDate)}
            </p>
          </div>
          <div className="bg-primary-50 p-4 rounded-lg">
            <h3 className="text-sm font-medium text-primary-500 mb-1">Grade Release</h3>
            <p className="text-sm text-primary-900">{formatDate(assignment.gradeReleaseDate)}</p>
          </div>
        </div>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-medium text-gray-900">{assignment.file.name}</p>
      ...
      {/* Grading Information */}
      <div className="bg-white rounded-xl shadow border border-primary-100 p-6">
        <h2 className="text-xl font-semibold text-primary-900 mb-4">Grading Information</h2>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="bg-primary-50 p-4 rounded-lg">
            <h3 className="text-sm font-medium text-primary-500 mb-1">Grading Type</h3>
            <p className="text-sm text-primary-900 capitalize">{assignment.grading?.type || 'N/A'}</p>
          </div>
          <div className="bg-primary-50 p-4 rounded-lg">
            <h3 className="text-sm font-medium text-primary-500 mb-1">Total Points</h3>
            <p className="text-sm text-primary-900">{assignment.grading?.total_points || 0} points</p>
          </div>
          <div className="bg-primary-50 p-4 rounded-lg">
            <h3 className="text-sm font-medium text-primary-500 mb-1">Submission Type</h3>
            <p className="text-sm text-primary-900 capitalize">{assignment.submissionType?.replace('_', ' ') || 'N/A'}</p>
          </div>
        </div>
        ...
        {assignment.grading?.method === 'rubric' && assignment.grading.rubric && (
          <div>
            <h3 className="text-lg font-semibold text-primary-900 mb-4">Grading Rubric</h3>

            {/* Performance Levels */}
            <div className="mb-4">
              <h4 className="text-md font-medium text-primary-700 mb-2">Performance Levels</h4>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                {assignment.grading.rubric.level?.sort((a, b) => b.rank - a.rank).map((level, index) => (
                  <div key={index} className="bg-primary-50 p-3 rounded-lg text-center">
                    <div className="text-sm font-medium text-primary-900">{level.description}</div>
                    <div className="text-xs text-primary-500">Level {level.rank}</div>
                  </div>
                ))}
              </div>
            </div>
              ...
              <table className="w-full border border-primary-300 rounded-lg">
                <thead>
                  <tr className="bg-primary-50">
                    ...
                    {assignment.grading.rubric.level?.sort((a, b) => b.rank - a.rank).map((level, index) => (
                      <th key={index} className="border-b border-primary-300 px-4 py-3 text-center text-sm font-medium text-primary-900">
                        {level.description}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {assignment.grading.rubric.criteria?.map((criteria, index) => (
                    <tr key={index} className="border-b border-primary-200">
                      <td className="px-4 py-3 text-sm text-primary-900">{criteria.description}</td>
                      <td className="px-4 py-3 text-sm text-primary-900 font-medium">{criteria.points}</td>
      ...
      {assignment.topics && assignment.topics.length > 0 && (
        <div className="bg-white rounded-xl shadow border border-primary-100 p-6">
          <h2 className="text-xl font-semibold text-primary-900 mb-4">Topics Covered</h2>
      ...
      {/* Submission Section */}
      <div className="bg-white rounded-xl shadow border border-primary-100 p-6">
        <h2 className="text-xl font-semibold text-primary-900 mb-4">Submit Assignment</h2>
        ...
        {assignment.submissionType === 'file_upload' ? (
          <div className="space-y-4">
            <div className="border-2 border-dashed border-primary-300 rounded-lg p-6 text-center">
              <svg className="mx-auto h-12 w-12 text-primary-400 mb-4" stroke="currentColor" fill="none" viewBox="0 0 48 48">
                <path d="M28 8H12a4 4 0 00-4 4v20m32-12v8m0 0v8a4 4 0 01-4 4H12a4 4 0 01-4-4v-4m32-4l-3.172-3.172a4 4 0 00-5.656 0L28 28M8 32l9.172-9.172a4 4 0 015.656 0L28 28m0 0l4 4m4-24h8m-4-4v8m-12 4h.02" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" />
              </svg>
              <p className="text-primary-600 mb-2">Drag and drop your file here, or click to browse</p>
              <p className="text-sm text-primary-500">Supported formats: PDF, DOC, DOCX</p>
            </div>
            <div className="flex gap-2">
