  ...
  return (
    <div className="bg-yellow-100 rounded-lg shadow-md p-4">
      ...
      <div className="grid grid-cols-2 grid-rows-3 gap-3 auto-rows-fr">
        {options.map((opt) => (
          <PrimaryButton asChild key={opt.id} className="flex items-center gap-3 text-sm h-full min-h-[3.5rem] w-full">
            <Link href={opt.href}>
              <span className="text-2xl text-yellow-900">{opt.icon}</span>
              <span className="text-left text-yellow-900 line-clamp-2 overflow-hidden">{opt.label}</span>
            </Link>
          </PrimaryButton>
        ))}
      </div>
