...
import { DashboardLayout, UserData } from "@/lib/schemas/database";
...
const SourceDraggableElement = ({
  ...
  return (
    ...
    >
      {
        DASHBOARD_ELEMENT_TO_NAME[
          elementType as keyof typeof DASHBOARD_ELEMENT_TO_NAME
        ]
      }
    </div>
  );
...
const DroppableColumn = ({
  ...
  isEmpty: boolean;
  activeColumn: "left" | "right" | null;
}) => {
  const { setNodeRef, isOver } = useDroppable({ id });

  const isColumnActive =
    (id === "left-column-container" && activeColumn === "left") ||
...
export default function DashboardEditModal({
  ...
  const roleAllowedElementTypes =
    dashboardAcl.role_restrictions[
      userRole as keyof typeof dashboardAcl.role_restrictions
    ] || [];
  ...
  useEffect(() => {
    if (isOpen && userId) {
      loadDashboardLayout(userData.dashboardLayout);
    }
  }, [isOpen, userId, userData]);

  const loadDashboardLayout = async (layout: DashboardLayout) => {
    setLoading(true);
    try {
      let globalCounter = 0;
  ...
  const resetToDefault = () => {
    ...
    try {
      ...
      const layout = defaultLayout[
        userRole as keyof typeof defaultLayouts.dashboardLayout
      ] || {
        leftColumn: [],
        rightColumn: [],
      };
  ...
  const handleDragOver = (event: DragEndEvent) => {
    ...
    if (
      over.id === "left-column-container" ||
      overData?.sortable?.containerId === "left-column-sorter"
    ) {
      targetColumnName = "left";
    } else if (
      over.id === "right-column-container" ||
      overData?.sortable?.containerId === "right-column-sorter"
    ) {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (isSourceItem && elementTypeFromSource) {
      ...
      if (targetIndex === undefined) {
        targetIndex =
          targetColumnName === "left" ? leftColumn.length : rightColumn.length;
      }
  ...
  if (loading) {
    return (
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="max-w-4xl border-2 border-primary-400 bg-primary-100">
          <DialogHeader>
          ...
          <div className="flex items-center justify-center p-8">
            <span
              className="inline-block w-10 h-10 border-4 border-primary-400 border-t-transparent rounded-full animate-spin"
              aria-label="Loading"
            />
          </div>
        </DialogContent>
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-6xl max-h-[90vh] flex flex-col border-2 border-primary-400 bg-primary-100">
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                <DroppableColumn
                  ...
                  isEmpty={leftColumn.length === 0}
                  activeColumn={activeColumn}
                >
                  <SortableContext
                    id="left-column-sorter"
                  ...
                  >
                    {leftColumn.map((item) => (
                      <DraggableItem
                        ...
                        label={
                          DASHBOARD_ELEMENT_TO_NAME[
                            item.type as keyof typeof DASHBOARD_ELEMENT_TO_NAME
                          ]
                        }
              ...
              <div className="space-y-2">
                ...
                <DroppableColumn
                  ...
                  isEmpty={rightColumn.length === 0}
                  activeColumn={activeColumn}
                >
                  <SortableContext
                    id="right-column-sorter"
                ...
                {activeDragData ? (
                  activeDragData.data.current?.isSource ? (
                    ...
                    <DraggedItemOverlay
                      label={
                        DASHBOARD_ELEMENT_TO_NAME[
                          findItemById(activeDragData.id as string)?.item
                            .type as keyof typeof DASHBOARD_ELEMENT_TO_NAME
                        ] || "Unknown"
                      }
                      isDropAllowed={isDropCurrentlyAllowed}
                    />
