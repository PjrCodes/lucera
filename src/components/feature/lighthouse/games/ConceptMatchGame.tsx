...
const ConceptMatchGame: React.FC<ConceptMatchGameProps> = ({ onBack }) => {
  ...
  const conceptPairs: ConceptPair[] = [
    { id: 1, term: "Variable", definition: "A storage location with an associated name that contains data", course: "CS101" },
    { id: 2, term: "Function", definition: "A reusable block of code that performs a specific task", course: "CS101" },
    { id: 3, term: "Array", definition: "A collection of elements stored in contiguous memory locations", course: "CS101" },
    { id: 4, term: "Loop", definition: "A control structure that repeats a block of code", course: "CS101" },
    { id: 5, term: "Recursion", definition: "A programming technique where a function calls itself", course: "CS201" },
    { id: 6, term: "Class", definition: "A blueprint for creating objects in object-oriented programming", course: "CS201" },
  ];
  ...
  const getCardStyle = (id: number, type: "term" | "definition") => {
    const isSelected = type === "term" ? selectedTerm === id : selectedDefinition === id;
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className="text-center mb-8">
        <h1 className="text-3xl font-bold text-gray-800 mb-2">🧩 Concept Match</h1>
        <p className="text-gray-600">Match terms with their definitions. Click a term, then click its matching definition!</p>
      </div>
      ...
      {gameCompleted && (
        <div className="bg-luceragreen-1 border border-luceragreen-3 rounded-lg p-6 mb-6 text-center">
          ...
          <h2 className="text-2xl font-bold text-luceragreen-5 mb-2">Congratulations!</h2>
          <p className="text-luceragreen-4 mb-4">
            You completed the game with a score of {score} points in {Math.floor((Date.now() - startTime) / 1000)} seconds!
          </p>
          <button
            onClick={resetGame}
